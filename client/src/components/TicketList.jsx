...
import { Search, Eye, Filter, ArrowUpDown, Clock, MapPin, CheckCircle, ExternalLink } from 'lucide-react';
...
export default function TicketList({
  ...
  const filtered = potholes.filter(p => {
    ...
    if (searchTerm.trim()) {
      ...
      return matchTrack || matchRoad || matchCity || matchAuth;
    }
    return true;
  });
    ...
    <div className="glass-panel" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flex: '1 1 260px', width: '100%' }}>
          <div style={{ position: 'relative', width: '100%' }}>
            ...
            <input
              type="text"
              placeholder="Search by Tracking ID, Road, or Civic Authority..."
              value={searchTerm}
      ...
      {filtered.length === 0 ? (
        <div style={{ padding: '3rem 1rem', textAlign: 'center', color: 'var(--text-secondary)' }}>
          ...
          <p style={{ fontSize: '0.85rem', marginTop: '0.35rem' }}>Try clearing filters or run a scan in the Detection Studio.</p>
        </div>
      ) : (
        <div>
          ...
          <div className="table-responsive-wrapper">
            <table style={{ width: '100%', minWidth: 680, borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
              <thead>
